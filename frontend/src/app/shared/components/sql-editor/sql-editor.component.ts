...
export class SqlEditorComponent implements OnInit, OnDestroy, OnChanges {
    ...
    private getSuggestions(
        ...
        if (dotMatch) {
            ...
            const matchingTable = this.tables.find(
            ...
            if (matchingTable) {
                return matchingTable.columns
                    ...
                    .filter(
                        (s) =>
                            !partialColumn ||
                            s.label.toLowerCase().startsWith(partialColumn.toLowerCase()),
                    );
            }
            return [];
        }
        ...
        const tableContextRegex =
            /(?:FROM|JOIN|UPDATE|INTO|TABLE|SELECT|WHERE|GROUP\s+BY|ORDER\s+BY|ON|SET|INSERT)\s+(?:\w+\s*,\s*)*(\w*)$/i;
        ...
        if (tableMatch) {
            return this.tables
                ...
                .filter((s) => !currentWord || s.label.toLowerCase().startsWith(currentWord));
        }
        ...
        if (/SELECT\s+(?:\w+\s*,\s*)*(\w*)$/i.test(lineUntilPosition)) {
            suggestions.push(
                ...['COUNT', 'SUM', 'AVG', 'MIN', 'MAX', 'DISTINCT'].map((func) => ({
                    label: func,
    ...
    private getSnippets(): { label: string; insertText: string; documentation: string }[] {
        return [
            ...
            },
            {
                label: 'group-by',
                insertText:
                    'SELECT ${1:columns}, COUNT(*) as count\nFROM ${2:table}\nGROUP BY ${1:columns}\nHAVING COUNT(*) ${3:condition};',
    ...
    private validateSql(sql: string): ValidationError[] {
        ...
        const fromRegex =
            /\bfrom\s+([a-zA-Z_][a-zA-Z0-9_]*)(?:\s+(?:as\s+)?([a-zA-Z_][a-zA-Z0-9_]*))?\b/i;
        const joinRegex =
            /\bjoin\s+([a-zA-Z_][a-zA-Z0-9_]*)(?:\s+(?:as\s+)?([a-zA-Z_][a-zA-Z0-9_]*))?\b/gi;
        ...
        joinMatches.forEach((match) => {
            ...
            if (!this.tables.some((t) => t.name.toLowerCase() === tableName.toLowerCase())) {
                ...
                errors.push({
                    message: `Tabelle "${tableName}" existiert nicht in der Datenbank`,
                    startLineNumber: this.getLineNumberForPosition(
                        sql,
                        pos + match[0].indexOf(tableName),
                    ),
                    startColumn: this.getColumnForPosition(sql, pos + match[0].indexOf(tableName)),
                    endLineNumber: this.getLineNumberForPosition(
                        sql,
                        pos + match[0].indexOf(tableName) + tableName.length,
                    ),
                    endColumn: this.getColumnForPosition(
                        sql,
                        pos + match[0].indexOf(tableName) + tableName.length,
                    ),
                    severity: monaco.MarkerSeverity.Error,
        ...
        while ((match = tableColumnRegex.exec(sql)) !== null) {
            ...
            if (!tableAliases.has(lowerTableName)) {
                errors.push({
                    ...
                    endLineNumber: this.getLineNumberForPosition(
                        sql,
                        match.index + referenceTableName.length,
                    ),
                    endColumn: this.getColumnForPosition(
                        sql,
                        match.index + referenceTableName.length,
    ...
    private findColumnReferences(
        sql: string,
    ): Array<{ column: string; table: string; position: number }> {
