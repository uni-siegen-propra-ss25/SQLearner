...
import {
    ...
    DataEditDialogData,
} from '../../dialogs/data-edit-dialog/data-edit-dialog.component';
import {
    EditTableDialogComponent,
    EditTableDialogData,
} from '../../dialogs/edit-table-dialog/edit-table-dialog.component';
...
export class DatabaseTableViewerComponent implements OnInit, AfterViewInit {
    ...
    loadTableRowCounts() {
        ...
        const promises = this.tables.map((table) =>
            this.databaseService
                .runQuery(this.databaseId!, `SELECT COUNT(*) as row_count FROM "${table.name}"`)
                .toPromise(),
        );
    ...
    loadTableData(tableName: string) {
        ...
        this.databaseService
            .runQuery(this.databaseId, `SELECT * FROM "${tableName}" LIMIT 100`)
            .subscribe({
                next: (result: QueryResult) => {
                ...
                error: (error: any) => {
                    ...
                    this.snackBar.open(
                        `Fehler beim Laden der Tabellendaten für ${tableName}.`,
                        'OK',
                        { duration: 3000 },
                    );
                    this.isLoading = false;
    ...
    openAddDataDialog(): void {
        ...
        dialogRef.afterClosed().subscribe((result: any) => {
            if (result && result.success) {
                ...
                this.databaseService
                    .insertRow(this.database!.id, this.selectedTable!.name, result.data)
                    .subscribe({
                        next: () => {
                            this.snackBar.open('Datensatz erfolgreich hinzugefügt!', 'OK', {
                                duration: 3000,
                            });
                        ...
                        error: (error: any) => {
                            ...
                            this.snackBar.open('Fehler beim Hinzufügen des Datensatzes.', 'OK', {
                                duration: 3000,
                            });
                        },
                    });
    ...
    openEditDataDialog(rowData: any): void {
        ...
        dialogRef.afterClosed().subscribe((result: any) => {
            if (result && result.success) {
                ...
                const primaryKeyColumn = this.selectedTable!.columns.find(
                    (col) => col.isPrimaryKey,
                );
                if (!primaryKeyColumn) {
                    this.snackBar.open('Kein Primärschlüssel gefunden für Update.', 'OK', {
                        duration: 3000,
                    });
                    return;
                }
                ...
                this.databaseService
                    ...
                    .subscribe({
                        next: () => {
                            this.snackBar.open('Datensatz erfolgreich aktualisiert!', 'OK', {
                                duration: 3000,
                            });
                            this.loadTableData(this.selectedTable!.name); // Reload table data
                        },
                        error: (error: any) => {
                            ...
                            this.snackBar.open('Fehler beim Aktualisieren des Datensatzes.', 'OK', {
                                duration: 3000,
                            });
                        },
                    });
    ...
    truncateTable() {
        ...
        dialogRef.afterClosed().subscribe((result: boolean) => {
            if (result) {
                this.databaseService
                    .runQuery(this.databaseId!, `TRUNCATE TABLE "${this.selectedTable!.name}"`)
                    .subscribe({
                        next: () => {
                            this.snackBar.open('Tabelle erfolgreich geleert.', 'OK', {
                                duration: 3000,
                            });
                        ...
                        error: (error: any) => {
                            ...
                            this.snackBar.open('Fehler beim Leeren der Tabelle.', 'OK', {
                                duration: 3000,
                            });
                        },
                    });
    ...
    deleteTable(table: DatabaseTable) {
        ...
        dialogRef.afterClosed().subscribe((result: boolean) => {
            if (result) {
                this.databaseService
                    .runQuery(this.databaseId!, `DROP TABLE "${table.name}"`)
                    .subscribe({
                        next: () => {
                            this.snackBar.open('Tabelle erfolgreich gelöscht.', 'OK', {
                                duration: 3000,
                            });
                            this.loadTables();
                        ...
                        error: (error: any) => {
                            ...
                            this.snackBar.open('Fehler beim Löschen der Tabelle.', 'OK', {
                                duration: 3000,
                            });
                        },
                    });
    ...
    deleteRow(row: any) {
        ...
        dialogRef.afterClosed().subscribe((result: boolean) => {
            if (result) {
                ...
                if (!primaryKeyColumn) {
                    this.snackBar.open('Kein Primärschlüssel gefunden für Löschung.', 'OK', {
                        duration: 3000,
                    });
                    return;
                }
                ...
                this.databaseService
                    .deleteRow(this.databaseId!, this.selectedTable!.name, whereClause)
                    .subscribe({
                        next: () => {
                            this.snackBar.open('Zeile erfolgreich gelöscht!', 'OK', {
                                duration: 3000,
                            });
                            this.loadTableData(this.selectedTable!.name); // Reload table data
                        },
                        error: (error: any) => {
                            ...
                            this.snackBar.open('Fehler beim Löschen der Zeile.', 'OK', {
                                duration: 3000,
                            });
                        },
                    });
