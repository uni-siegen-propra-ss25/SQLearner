...
export class RoadmapService {
  ...
  createChapter(chapter: Partial<Chapter>): Observable<number> {
    return this.http.post<number>(`${this.baseUrl}/chapters`, chapter)
      .pipe(catchError(error => this.handleError(error)));
  }

  updateChapter(id: number, chapter: Partial<Chapter>): Observable<void> {
    return this.http.put<void>(`${this.baseUrl}/chapters/${id}`, chapter)
      .pipe(catchError(error => this.handleError(error)));
  }
  ...
  getTopic(chapterId: number, id: number): Observable<Topic> {
    return this.http.get<Topic>(`${this.baseUrl}/chapters/${chapterId}/topics/${id}`)
      .pipe(catchError(error => this.handleError(error)));
  }

  createTopic(chapterId: number, topic: Partial<Topic>): Observable<number> {
    return this.http.post<number>(`${this.baseUrl}/chapters/${chapterId}/topics`, topic)
      .pipe(catchError(error => this.handleError(error)));
  }

  updateTopic(chapterId: number, id: number, topic: Partial<Topic>): Observable<Topic> {
    return this.http.put<Topic>(`${this.baseUrl}/chapters/${chapterId}/topics/${id}`, topic)
      .pipe(catchError(error => this.handleError(error)));
  }

  deleteTopic(chapterId: number, id: number): Observable<void> {
    return this.http.delete<void>(`${this.baseUrl}/chapters/${chapterId}/topics/${id}`)
      .pipe(catchError(error => this.handleError(error)));
  }

  reorderTopics(chapterId: number, topics: { id: number; order: number }[]): Observable<void> {
    return this.http.put<void>(`${this.baseUrl}/chapters/${chapterId}/topics/reorder`, { topics })
      .pipe(catchError(error => this.handleError(error)));
  }

  // Exercise endpoints
  getExercises(topicId: number): Observable<Exercise[]> {
    return this.http.get<Exercise[]>(`${this.baseUrl}/topics/${topicId}/exercises`)
      .pipe(catchError(error => this.handleError(error)));
  }
