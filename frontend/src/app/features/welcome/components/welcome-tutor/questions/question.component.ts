...
export class QuestionComponent implements OnInit {
    ...
    ngOnInit() {
        this.questionService.getAll().subscribe((data) => {
            const ungefiltert = data.filter(
                (q) => !q.ist_archiviert && !q.ist_geloescht && !q.ist_beantwortet,
            );
    ...
    antwortAbschicken() {
        ...
        this.questionService
            .antworten(this.ausgewaehlteQuestion.id, this.antwortText)
            .subscribe(() => {
                this.questions = this.questions.filter(
                    (f) => f.id !== this.ausgewaehlteQuestion.id,
                );
                this.angepinnt = this.angepinnt.filter(
                    (f) => f.id !== this.ausgewaehlteQuestion.id,
                );
