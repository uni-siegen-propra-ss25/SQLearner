...
export async function main() {
    ...
    if (!sampleDb) {
        ...
        sampleDb = await prisma.database.create({
            data: {
                ...
                description:
                    'Eine einfache Universitätsdatenbank mit Studenten, Kursen und Professoren',
    ...
    if (!queryExercise) {
        queryExercise = await prisma.exercise.create({
            data: {
                ...
                title: 'Alle Studenten anzeigen',
                description:
                    'Schreiben Sie eine SQL-Abfrage, die alle Studenten aus der students-Tabelle anzeigt.',
    ...
    if (!complexQueryExercise) {
        complexQueryExercise = await prisma.exercise.create({
            data: {
                ...
                title: 'Studenten mit ihren Kursen',
                description:
                    'Erstellen Sie eine Abfrage, die alle Studenten mit ihren eingeschriebenen Kursen anzeigt. Zeigen Sie den Namen des Studenten und den Titel des Kurses an.',
                type: ExerciseType.QUERY,
    ...
    if (!freetextExercise) {
        freetextExercise = await prisma.exercise.create({
            data: {
                ...
                title: 'JOIN-Typen erklären',
                description:
                    'Erklären Sie den Unterschied zwischen INNER JOIN, LEFT JOIN und RIGHT JOIN. Geben Sie jeweils ein Beispiel an.',
    ...
    if (!existingSetting) {
        await prisma.settings.create({
            data: {
                ...
                description:
                    'OpenAI API Key for AI-powered features (SQL query assistance, feedback generation)',
            },
        });
    }
    ...
    const existingBookmarks = await prisma.bookmark.findMany({
        where: {
            OR: [{ userId: student.id }, { userId: student2.id }],
        },
    });
        ...
        await prisma.submission.createMany({
            data: [
                ...
                },
                {
                    ...
                    isCorrect: false,
                    feedback:
                        'Fast richtig! Die Aufgabe verlangt aber alle Spalten (*), nicht nur den Namen.',
                },
                {
                    userId: student.id,
                    exerciseId: complexQueryExercise.id,
    ...
    console.log({
        ...
        exercises: [
            {
                id: multipleChoiceExercise.id,
                title: multipleChoiceExercise.title,
                type: multipleChoiceExercise.type,
            },
            {
                id: singleChoiceExercise.id,
                title: singleChoiceExercise.title,
                type: singleChoiceExercise.type,
            },
            { id: queryExercise.id, title: queryExercise.title, type: queryExercise.type },
            {
                id: complexQueryExercise.id,
                title: complexQueryExercise.title,
                type: complexQueryExercise.type,
            },
