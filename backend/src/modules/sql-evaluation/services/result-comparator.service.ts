...
export class ResultComparatorService {
     ...
     * ```
     */
    compareResults(studentResult: QueryResult, expectedResult: QueryResult): ComparisonResult {
        this.logger.debug('Starting result comparison');

        const differences: ComparisonDifference[] = [];

        // 1. Compare columns
        const columnsMatch = this.compareColumns(studentResult, expectedResult, differences);

        // 2. Compare row count
        const rowCountMatch = this.compareRowCount(studentResult, expectedResult, differences);
        // 3. Compare data (only if structure matches)
        let dataMatches = false;
        if (columnsMatch && rowCountMatch) {
            dataMatches = this.compareData(studentResult, expectedResult, differences);
        }

        const isExactMatch = columnsMatch && rowCountMatch && dataMatches;

        this.logger.debug(
            `Comparison complete: match=${isExactMatch}, columns=${columnsMatch}, rows=${rowCountMatch}, data=${dataMatches}`,
        );
     ...
     */ private compareColumns(
        ...
        if (this.hasDuplicateColumns(studentColumns) || this.hasDuplicateColumns(expectedColumns)) {
            const duplicateSource = this.hasDuplicateColumns(studentColumns)
                ? 'student'
                : 'expected';
            differences.push({
                type: 'COLUMN_MISMATCH',
