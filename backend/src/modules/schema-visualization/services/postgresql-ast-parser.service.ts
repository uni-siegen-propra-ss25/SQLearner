...
export class PostgreSQLASTParser {
    ...
    private mapColumnDefinition(column: any, tableName: string): ColumnSchema {
        try {
            ...
            throw new ASTMappingError(
                `Failed to map column definition: ${error.message}`,
                tableName,
                column.name?.name,
            );
        }
    ...
    private extractForeignKeysFromTable(statement: any): ForeignKeySchema[] {
        ...
        for (const column of statement.columns || []) {
            ...
            for (const constraint of column.constraints || []) {
                if (constraint.type === 'foreign key') {
                    const fk = this.mapForeignKeyConstraint(
                        constraint,
                        tableName,
                        column.name?.name,
                    );
        ...
        for (const constraint of statement.constraints || []) {
            if (constraint.type === 'foreign key') {
                ...
                const sourceColumns = constraint.localColumns
                    ?.map((col) => col.name)
                    .filter((name) => name);
                const targetTable = this.getTableNameOnly(constraint.foreignTable);
                const targetColumns = constraint.foreignColumns
                    ?.map((col) => col.name)
                    .filter((name) => name);
                ...
                if (sourceColumns?.length && targetTable && targetColumns?.length) {
                    ...
                    if (sourceColumns.length !== targetColumns.length) {
                        console.warn(
                            `⚠️  Warning: Foreign key column count mismatch: ${sourceColumns.length} source columns vs ${targetColumns.length} target columns`,
                        );
     ...
     */
    private mapForeignKeyConstraint(
        constraint: any,
        sourceTable: string,
        sourceColumn: string,
        ...
        try {
            ...
            const targetTable = this.getTableNameOnly(
                constraint.foreignTable || constraint.references?.table,
            );
            const targetColumn =
                constraint.foreignColumns?.[0]?.name || constraint.references?.columns?.[0]?.name;
    ...
    private extractErrorFragment(ddl: string, location: any): string {
        try {
            ...
            return line
                ? `Line ${lineIndex + 1}: ${line.substring(Math.max(0, column - 20), column + 20)}`
                : '';
        } catch {
            return '';
        }
    ...
    private markForeignKeyColumns(tables: TableSchema[], foreignKeys: ForeignKeySchema[]): void {
        ...
        for (const fk of foreignKeys) {
            ...
            if (!targetTable) {
                console.warn(
                    `⚠️  Warning: Target table '${fk.targetTable}' not found. FK will be processed but may be invalid.`,
                );
            } else {
                ...
                for (const targetColumnName of targetColumns) {
                    ...
                    if (!targetColumn) {
                        console.warn(
                            `⚠️  Warning: Target column '${targetColumnName}' not found in table '${fk.targetTable}'. FK will be processed but may be invalid.`,
                        );
                    }
                }
            }
